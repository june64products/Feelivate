    ...
    if week_report_data and isinstance(week_report_data, dict):
        ...
        if _qw:
            system_content += (
                ...
                f"\n  • SAME LEVEL ≠ SAME DAY COUNT. Week {wn} may have been a short,"
                f" mid-week window; this one runs from TODAY through SUNDAY and can be"
                f" longer. Re-fit the same content across THIS window — one entry per"
                f" calendar day, last entry Sunday. Never copy Week {wn}'s day count and"
                f" stop early; spread the work out or add explicit rest days to fill it."
